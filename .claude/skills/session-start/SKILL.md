...
- If on `main`, the working tree is clean, and `git log HEAD..origin/main` showed commits (local is behind, not ahead): `git pull --ff-only`. This is the other developer's merged work — always start the session on top of it rather than a stale snapshot. `--ff-only` is deliberate: since `main` is protected (no direct pushes, PRs only) local `main` should never diverge from `origin/main` with commits of its own, so a fast-forward should always be possible. If it isn't, don't force it — surface that as unexpected and ask before doing anything destructive. Skip the pull (and say why) if there are uncommitted changes, if local is ahead/diverged, or if currently on a feature branch — pulling in those cases risks stomping in-progress work or pulling the wrong branch.
